...
final class ImportLegacySocialProfilesAction
{
    ...
    public function handle(?int $siteId = null, bool $dryRun = false): LegacySocialImportResultData
    {
        ...
        foreach ($sites as $site) {
            ...
            $hasConfiguration = SocialProfile::query()->where('site_id', $currentSiteId)->exists()
                || SocialSitePreferences::query()->where('site_id', $currentSiteId)->exists();

            if ($hasConfiguration) {
                $skipped[] = sprintf('Site %d already has a Socials configuration.', $currentSiteId);
