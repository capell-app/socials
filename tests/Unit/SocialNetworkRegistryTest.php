...
it('keeps networks without stable identity support follow-only', function (): void {
    ...
    foreach (['instagram', 'youtube', 'tiktok', 'mastodon', 'threads'] as $key) {
        expect($definitions[$key]->supports(SocialNetworkCapability::Follow))->toBeTrue()
            ->and($definitions[$key]->supports(SocialNetworkCapability::SameAs))->toBeFalse()
            ->and($definitions[$key]->supports(SocialNetworkCapability::Share))->toBeFalse();
    }
});
...
it('keeps the current definition when an override is invalid', function (): void {
    ...
    expect(fn (): mixed => (new OverrideSocialNetworkDefinitionAction)->handle(
