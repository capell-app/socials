...
it('declares the shipped Socials package surfaces and Marketplace assets', function (): void {
    ...
    if (! is_array($providers) || ! is_array($traceability)) {
        throw new RuntimeException('Socials manifest provider and traceability metadata must be objects.');
    }

    $adminPageContribution = collect($contributions)
        ->first(static fn (array $contribution): bool => ($contribution['class'] ?? null) === SocialsAdminPageContribution::class);
    $security = $manifest['security'] ?? null;
    $adminSurface = is_array($security) ? $security['adminSurface'] ?? null : null;

    if (! is_array($adminPageContribution) || ! is_array($adminSurface)) {
        throw new RuntimeException('Socials admin authorization metadata must be objects.');
    }
